...
public class PlayerController : MonoBehaviour
{
    ...
    [SerializeField]
    public int gamepad_id;

    [SerializeField] protected Transform cursorTF;
    ...
    public void OnMoveCursor(InputAction.CallbackContext context)
    {
        ...
        //if device from context doesnt match the device associated with this gameobject, return
        ...
        if (deflating)
        {
            ...
            if (fartMeter > 0)
            {
                ...
                if (farting)
                {
                    ...
                    Vector2 cursorPos = cursorTF.position;
                    Vector2 diff = new Vector2(transform.position.x, transform.position.y) - cursorPos;
                    diff.Normalize();
    ...
    public void OnTickle(InputAction.CallbackContext context)
    {
        ...
        if (canTickle)
        {
            ...
            Vector2 hit_point = cursorTF.position;
            RaycastHit2D hit = Physics2D.Raycast(hit_point, Vector2.zero);
            if (hit)
            {
